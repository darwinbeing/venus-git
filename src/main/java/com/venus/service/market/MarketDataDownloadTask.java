...
public class MarketDataDownloadTask implements Runnable{
    ...
    @Override
    public void run() {
        //String url = marketDataService.extractURL(this.stockName, this.timePeriod,this.historyDays,isDividends);
        //String todayUrl = new YahooMarketDataService().extractTodayURL(this.stockName);

        //this.downloadMarketDataToFile(url, this.fileName);
        //this.downloadMarketDataToFile(todayUrl, this.fileName);

        this.downloadMarketDataToDB();
    }

    public void downloadMarketDataToDB(){
        this.marketService.updateMarketData(this.code);
    }
